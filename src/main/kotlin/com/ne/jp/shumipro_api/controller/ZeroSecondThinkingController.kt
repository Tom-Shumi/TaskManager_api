...
class ZeroSecondThinkingController: BaseController() {
    ...
    fun getZeroSecondThinkingList(@AuthenticationPrincipal loginUser: ShumiproLoginUser,
        ...
        val zeroSecondThinkingList = zeroSecondThinkingService.getZeroSecondThinkingList(loginUser, nextKey)
        return if (zeroSecondThinkingList is List<ZeroSecondThinkingDto>){
            // タスクコメント取得成功
     ...
     */
    @PostMapping
    fun registerZeroSecondThinking(@Validated @RequestBody taskCommentRequest: TaskCommentRequest,
